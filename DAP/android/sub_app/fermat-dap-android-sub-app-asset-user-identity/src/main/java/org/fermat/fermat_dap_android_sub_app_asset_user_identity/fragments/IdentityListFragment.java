...
public class IdentityListFragment extends FermatListFragment<IdentityAssetUser>
        ...
        try {
            ...
            newIdentityButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    changeActivity(Activities.DAP_SUB_APP_ASSET_USER_IDENTITY_CREATE_IDENTITY.getCode(), appSession.getAppPublicKey());
                }
            });
            ...
            if (moduleManager.getIdentityAssetUsersFromCurrentDeviceUser().size() > 0)
    ...
    protected int getLayoutResource() {
        return R.layout.fragment_dap_user_identity_list;
    }
    ...
    public FermatAdapter getAdapter() {
        if (adapter == null) {
            adapter = new UserIdentityAdapter(getActivity(), identityInformationList);
    ...
    @Override
    public List<IdentityAssetUser> getMoreDataAsync(FermatRefreshTypes refreshType, int pos) {
        List<IdentityAssetUser> data = new ArrayList<>();
    ...
    @Override
    public void onItemClickListener(IdentityAssetUser data, int position) {

    }

    @Override
    public void onLongItemClickListener(IdentityAssetUser data, int position) {
