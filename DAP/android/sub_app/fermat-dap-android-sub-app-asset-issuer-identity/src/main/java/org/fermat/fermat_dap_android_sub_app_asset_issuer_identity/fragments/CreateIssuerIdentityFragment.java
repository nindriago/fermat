                ...
                try {
                    if (!isUpdate) {
                        executorService.submit(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    moduleManager.createNewIdentityAssetIssuer(brokerNameText, (brokerImageByteArray == null) ? convertImage(R.drawable.profile_actor) : brokerImageByteArray);
                                        ...
                                        identityIssuerDialogCropImage.setOnDismissListener(new DialogInterface.OnDismissListener() {
                                            ...
                                            public void onDismiss(DialogInterface dialog) {
                                                if (identityIssuerDialogCropImage.getCroppedImage() != null) {
                                                    mIdentityBitmap = getResizedBitmap(rotateBitmap(identityIssuerDialogCropImage.getCroppedImage(), ExifInterface.ORIENTATION_NORMAL), 230, 230);
                                ...
                                identityIssuerDialogCropImagee.setOnDismissListener(new DialogInterface.OnDismissListener() {
                                    ...
                                    public void onDismiss(DialogInterface dialog) {
                                        if (identityIssuerDialogCropImagee.getCroppedImage() != null) {
                                            mIdentityBitmap = getResizedBitmap(rotateBitmap(identityIssuerDialogCropImagee.getCroppedImage(), ExifInterface.ORIENTATION_NORMAL), 230, 230);
                                            Picasso.with(getActivity()).load(getImageUri(getActivity(), mIdentityBitmap)).transform(new CircleTransform()).into(mIdentityImage);
                                            brokerImageByteArray = ImagesUtils.toByteArray(mIdentityBitmap);
                                            updateProfileImage = true;
                                        } else {
